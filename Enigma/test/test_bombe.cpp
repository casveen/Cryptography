...
bool is_exact_setting (Enigma &enigma,
                       vector<struct EnigmaSetting> solutions,
                       string ring_setting,
                       string rotor_position,
                       string correct_plugboard_string,
                       string ciphertext,
                       string crib,
                       int crib_pos) {
    ...
    for (struct EnigmaSetting solution : solutions) {
        ...
        if (correct_plugboard_string != "") {
            ...
            for (int i = 0; i < 26; ++i) {
                ...
                if (check == false && solution_count == 1) {
                    cerr << "WRONG PLUGBOARD: correct_plugboard[" << i << "]=" << correct_plugboard.get_wiring (i)
...
SCENARIO ("Configuration grid returns equivalent solutions", "[Configuration Grid]") {
    GIVEN ("Configuration Grid") {
        Enigma enigma ({ VI, II, VIII }, UKWR);
        ...
        ConfigurationGrid configuration_grid (enigma);
        configuration_grid.set_crib_length (12);
        /*vector<pair<string, string>> equivalents = configuration_grid.get_equivalents ("AAA", "AAA");
        ...
        WHEN ("Running bombe close to configuration") {
            THEN ("Bombe should find the configuration") {
                ...
                CHECK (is_exact_setting (enigma, solutions, "EPEL", "CDTJ", "AE.BF.CM.DQ.HU.JN.LX.PR.SZ.VW", ciphertext, crib, 0));
            }
        }*/
    }
}
    ...
    GIVEN ("Enigma with rotors I, II, III, reflector UKW and a long plaintext") {
        ...
        WHEN ("Given ciphertext encrypted with ring-setting AAA, "
            ...
            THEN ("Running bombe with a complete crib should return the above "
                  "setting") {
                vector<struct EnigmaSetting> solutions = bombe.analyze (ciphertext, { plaintext });
                CHECK (is_exact_setting (enigma, solutions, "AAA", "AAA", "", ciphertext, plaintext, 0));
            }
        }
        ...
        WHEN ("Given ciphertext encrypted with ring-position CAA and no "
            ...
            THEN ("Running bombe with a complete crib should return the above "
                  "setting") {
                vector<struct EnigmaSetting> solutions = bombe.analyze (ciphertext, { plaintext });
                CHECK (is_exact_setting (enigma, solutions, "AAA", "CAA", "", ciphertext, plaintext, 0));
            }
        }
        ...
        WHEN ("Given ciphertext encrypted with ring-setting FGH, ring-position "
            ...
            THEN ("Running bombe with a complete crib should return the above "
                  "setting") {
                vector<struct EnigmaSetting> solutions = bombe.analyze (ciphertext, { plaintext });
                CHECK (is_exact_setting (enigma, solutions, "FGH", "CDE", "", ciphertext, plaintext, 0));
            }
        }
        ...
        WHEN ("Given ciphertext encrypted with ring-setting RFW, ring-position "
            ...
            THEN ("Running bombe with a complete crib should return the above "
                  "setting") {
                vector<struct EnigmaSetting> solutions = bombe.analyze (ciphertext, { plaintext });
                CHECK (is_exact_setting (enigma, solutions, "BBA", "MCW", "AC. BD. EG. IR. JT. QZ", ciphertext, plaintext, 0));
            }
        }
        ...
        WHEN ("Given ciphertext encrypted with ring-setting AFG, rotor-position "
            ...
            THEN ("Running bombe with a complete crib should return the above "
                  "setting") {
                vector<struct EnigmaSetting> solutions = bombe.analyze (ciphertext, { plaintext });
                CHECK (is_exact_setting (enigma, solutions, "AFG", "HAP", "AK. IE. DV. CQ. BN, MO, PJ. WR. UX",
    ...
    GIVEN ("Enigma: VI, I, VI, UKWR. Steckered") {
        ...
        WHEN ("ciphertext encrypted with RS:GHK, RP:BLO") {
            ...
            THEN ("Running bombe with a complete crib should return the above "
                  "setting") {
                vector<struct EnigmaSetting> solutions = bombe.analyze (ciphertext, { plaintext });
                CHECK (is_exact_setting (enigma, solutions, "GHK", "BLO", "AK. IE. DV. CQ. BN, MO, PJ. WR. UX",
                                         ciphertext, plaintext, 0));
            }
        }
    }
