...
class ConfigurationGrid {
    ...
    // unsigned int get_unique_configurations() const;
...
class BombeUnit {
    ...
    BombeUnit (const vector<Rotor> rotors, const Reflector reflector, const bool use_configuration_board = DEFAULT_USE_CONFIGURATION_GRID);
    BombeUnit (struct EnigmaSetting, const bool use_configuration_board = DEFAULT_USE_CONFIGURATION_GRID);
    ~BombeUnit ();
    // setters
...
struct BombeSetting {
    ...
    string solution_file = "solutions.txt";
};
...
class Bombe {
    ...
    bool m_print_solution_cascade = false;

    public:
    Bombe (const initializer_list<Rotor> rotors, const Reflector reflector, const bool use_configuration_grid = DEFAULT_USE_CONFIGURATION_GRID);
    Bombe (vector<Rotor> rotors, vector<Reflector> reflector, const bool use_configuration_grid = DEFAULT_USE_CONFIGURATION_GRID);
    // Bombe(struct EnigmaSetting enigma_setting);
    vector<struct EnigmaSetting> analyze_unit (const string &, const string &, vector<Rotor> &, Reflector &, int, int);
    vector<int> probable_search (const string &, const string &);
    vector<struct EnigmaSetting> analyze (const string &, const vector<string> &);
