...
void vector_from_array_inplace(vector<shint> &vec, const int *array,
                               int length) {
...
void BombeUnit::init_enigma_encryptions(int encryptions,
  ...
  positions.clear();
  positions.push_back(
      vector_from_array(m_enigma->get_positions(), m_rotor_count));
  for (int i = 0; i < encryptions; ++i) {
    ...
    rotor_positions.push_back(m_enigma->get_rotor_position_as_string());
    positions.push_back(
        vector_from_array(m_enigma->get_positions(), m_rotor_count, true));
  }
}
...
vector<struct EnigmaSetting> BombeUnit::analyze(const string &ciphertext,
  ...
  int crib_n = crib.length(),
      total_permutations =
          m_enigma->compute_total_permutations_brute_force() + crib_n - 1,
      ring_settings =
          min((int)pow(m_letters, m_rotor_count), m_setting.max_ring_settings);
  if (m_use_configuration_grid) {
    ring_settings /=
        m_letters; // ignore slowest rotor TODO make use of in CG, to alloc less
  }
      ...
      if (!m_use_configuration_grid ||
          (m_use_configuration_grid &&
           !m_configuration_grid->get_checked(
               ...
               vector_from_string(
                   rotor_positions[rotor_positions.size() - crib_n - 1])))) {
        ...
        if (check_one_wire(most_wired_letter)) { // first test
          if (doublecheck_and_get_plugboard()) { // second test
            if (tripplecheck(crib, ciphertext, position,
              ...
              /*cout << "\nRS:" << m_enigma->get_ring_setting_as_string() << "
                 RP:"
                   << rotor_positions[rotor_positions.size() - crib.length() -
                 1]
                   << "   P:" <<
                 m_enigma->get_cartridge()->get_positions_as_string()
                   << "\n";*/
        ...
        if (m_use_configuration_grid) {
          m_configuration_grid->set_checked(positions.end() - crib.length() -
                                            1);
        }
      }
...
bool BombeUnit::tripplecheck(const string &crib, const string &ciphertext,
                             int candidate, vector<string> &rotor_positions) {
...
// bool compare_enigmaSetting(struct EnigmaSetting setting1, struct
// EnigmaSetting setting2, int length) TODO find if equivalent over a given
// length
...
Bombe::Bombe(const initializer_list<Rotor> rotors, const Reflector reflector,
    ...
    : m_rotors{vector<Rotor>(rotors)},
      m_reflector{vector<Reflector>({reflector})}, m_use_configuration_grid{
                                                       use_configuration_grid} {
...
Bombe::Bombe(vector<Rotor> rotors, vector<Reflector> reflector,
             const bool use_configuration_grid)
    : m_rotors(rotors),
      m_reflector(reflector), m_use_configuration_grid{use_configuration_grid} {
  m_letters = m_rotors[0].get_wires();
}

vector<int> Bombe::probable_search(const string &ciphertext,
                                   const string &crib) {
...
vector<struct EnigmaSetting>
Bombe::analyze_unit(const string &ciphertext_substring, const string &crib,
                    vector<Rotor> &rotor_configuration, Reflector &reflector,
                    int candidate, int most_wired_letter) {
  BombeUnit unit(rotor_configuration, reflector, m_use_configuration_grid);
  auto start_unit_run = std::chrono::system_clock::now();
  unit.set_identifier(unit.get_identifier() + " position " +
                      to_string(candidate) + " ");
  // translate settings
  unit.get_setting().performance_ring_setting_mean =
      m_setting.performance_ring_setting_mean;
  unit.get_setting().performance_ring_setting_var =
      m_setting.performance_ring_setting_var;
  ...
  unit.get_setting().starting_rotor_positions =
      m_setting.starting_rotor_positions;
  unit.get_setting().only_one_candidate = m_setting.only_one_candidate;
  ...
  if (m_setting.time_performance) {
    ...
    update_performance(
        m_setting.performance_unit_run_mean, m_setting.performance_unit_run_var,
        stop_unit_run - start_unit_run, m_setting.records_unit_run);
    // std::chrono::duration<double> measurement= (stop_unit_run -
    // start_unit_run); cout << " unit run: " << measurement.count();
  }
...
int Bombe::find_most_wired_letter(const string &ciphertext,
                                  const string &crib) {
...
vector<struct EnigmaSetting> Bombe::analyze(const string &ciphertext,
  ...
  vector<vector<Rotor>> rotor_configurations =
      choices_of(m_rotors, m_setting.rotor_count);
  if (m_setting.only_one_configuration) {
    rotor_configurations.erase(rotor_configurations.begin() + 1,
                               rotor_configurations.end());
  }

  // spawn units that analyze. For each candidate, for each configuration of
  // rotors
  // TODO thread unsaefe!!!
  ...
  for (i = 0; i < candidates.size(); ++i) {
    for (k = 0; k < m_reflector.size(); ++k) {
      for (j = 0; j < rotor_configurations.size(); ++j) {
        ...
        string ciphertext_substring =
            ciphertext.substr(candidate, crib.length());
        int most_wired_letter =
            find_most_wired_letter(ciphertext_substring, crib);
        // TODO common
        vector<Rotor> rotor_configuration = rotor_configurations[j];
        // TODO thread it
        vector<struct EnigmaSetting> solutions_unit =
            analyze_unit(ciphertext_substring, crib, rotor_configuration,
                         m_reflector[k], candidate, most_wired_letter);
...
/*If a message of length m is encrypted with a particular ring setting and rotor
posiiton, then encrypting the same message with rotor position turned one step
forward and ring setting one step "backward"(compensating for turn of rotor)
should give the exact same message unless there was a notch turnover when
encrypting. That is, there are a lot of settings of the enigma that would
encrypt to the exact same message.*/
ConfigurationGrid::ConfigurationGrid(Enigma &enigma) {
  ...
  try {
    ...
    // larger than it needs to be...only
    // m_all_rotor_poositions.size()<<letter^rotors
    m_all_rotor_positions_inverse =
        vector<unsigned int>(pow(m_letters, m_rotor_count), 0);
  } catch (bad_alloc &ba) {
    cerr << "ERROR:Failed to allocate inverse of all rotor positions, disable "
            "configuration "
            "grid";
  }
  cout << "DONE\n";
  try {
    cout << "\rAllocating checked, "
         << m_rotor_configuration_count * pow(m_letters, m_rotor_count)
         << " bits="
         << ((m_rotor_configuration_count * pow(m_letters, m_rotor_count)) /
             (8 * 125000000))
         << " Gb\n";
    m_checked = vector<bool>(
        m_rotor_configuration_count * pow(m_letters, m_rotor_count), false);
  ...
  m_total_configurations =
      pow(m_letters, m_rotor_count) * m_rotor_configuration_count;
}
...
bool ConfigurationGrid::get_checked(const int *ring_setting,
                                    const vector<int> &rotor_position) {
  // translate to index
...
void ConfigurationGrid::set_checked(
  ...
  for (unsigned int rpp = 0; rpp < m_rotor_configuration_count; ++rpp) {
    ...
    if (!m_checked[rss * m_rotor_configuration_count + rpp]) { // if not checked
      ...
      // positions_origina return if the configuration we are looking at equals
      // the original
      equal = [&]() { // lambda, so as to break out of both loops quickly with
                      // return
        for (int i = 1; i < m_rotor_count;
             ++i) { // note i=1, fast rotors are always equal
          for (int p = 1; p < m_crib_length;
               ++p) { // from p=1; first pos equal per def
            position_p_i =
                (m_all_rotor_positions[(rpp + p) % m_rotor_configuration_count]
                                      [i] -
...
void ConfigurationGrid::find_unchecked() const {
  // after an entire run there should not be any unchecked configurations. If
  // Ther are any, print them and issue a warning
  if (m_total_configurations > m_checked_configurations) {
    ...
    for (int rs = 0; rs < pow(m_letters, m_rotor_count); ++rs) {
      for (unsigned int rp = 0; rp < m_rotor_configuration_count; ++rp) {
        if (!m_checked[rs * m_rotor_configuration_count + rp]) {
          ...
          rss = rs;
          for (int i = 0; i < m_rotor_count; ++i) {
            cout << (m_all_rotor_positions[rp][i] - rss % m_letters +
                     m_letters) %
                        m_letters;
    ...
    if (missing_counter > 0) {
      cout << "In total, missing "
           << m_total_configurations - m_checked_configurations
           << " configurations";
    }
  }
  return;
...
unsigned int
ConfigurationGrid::rotor_position_string_to_int(const string &rotor_position) {
  ...
  // here we use the array m_all_rotor_positions_inverse, but first we have make
  // an index from the string, to look up in the inverse array
  return m_all_rotor_positions_inverse[string_to_int_hash(rotor_position)];
}
unsigned int
ConfigurationGrid::ring_setting_array_to_int(const int *ring_setting) {
  // a injective mapping from ring setting strings to int. Essentially a hash
  // when rs is an array, it is given in to opposite order of all other states,
  // so use reverse hash
  return array_to_int_reverse_hash(ring_setting);
}
unsigned int
ConfigurationGrid::rotor_position_array_to_int(const int *rotor_position) {
  // a injective mapping from ring setting strings to int. Essentially a hash
  // must map correctly to m_all_rotor_positions
  // that is,
  // m_all_rotor_positions[rotor_position_string_to_int[rotor_position]]=rotor_position
  // here we use the array m_all_rotor_positions_inverse, but first we have make
  // an index from the string, to look up in the inverse array
  return m_all_rotor_positions_inverse[array_to_int_reverse_hash(
      rotor_position)];
}
unsigned int ConfigurationGrid::rotor_position_vector_to_int(
    const vector<int> &rotor_position) {
  // a injective mapping from ring setting strings to int. Essentially a hash
  // must map correctly to m_all_rotor_positions
  // that is,
...
unsigned int
ConfigurationGrid::ring_setting_string_to_int(const string &ring_setting) {
  // a injective mapping from ring setting strings to int. Essentially a hash
